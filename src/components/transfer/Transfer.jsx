...
import { transfer } from '../../lib/http/users';
...
function Transfer() {
  ...
  const { status, error, data } = useQuery('transfer', transfer);
  ...
  if (status !== 'success') {
    return <ErrorPage status={status} error={error} data={data} title="Transfer" />;
  }

  return (
    <Pageview title="Transfer">
